...
# ctranslate2 (faster-whisper) and torch (pyannote) each bundle their own copy of
# libiomp5 (OpenMP). When both load in the same process on macOS:
# - KMP_DUPLICATE_LIB_OK suppresses the immediate abort
# - OMP_NUM_THREADS=1 prevents both runtimes from spawning parallel worker threads,
#   which would corrupt each other's state and cause SIGSEGV crashes
